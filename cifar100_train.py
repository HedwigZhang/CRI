from sgrise import designdeNetwork
...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        # inference located in resnet.py, main function is to set the resnet
        logits = designdeNetwork(self.image_placeholder, reuse=False)
        vali_logits = designdeNetwork(self.vali_image_placeholder, reuse=True)
        #self.tem_loss = 20
        ...
        # I want to increase this design that the network can 
        #if self.tem_loss < self.full_loss:
        #    self.full_loss = self.tem_loss
    ...
    def train(self):
        ...
        '''
        # For the first step, we are loading all training images and validation images into the memory
        ...
        for step in range(FLAGS.train_steps):
            ...
            if step % FLAGS.report_freq == 0:
                if FLAGS.is_full_validation is True:
                    ...
                    vali_summ.Value(tag='full_validation_error', simple_value=validation_error_value.astype(np.float))
            ...
            if step == FLAGS.decay_step0 or step == FLAGS.decay_step1:
                #FLAGS.init_lr = ((step / FLAGS.train_steps)**0.9) * FLAGS.init_lr
                FLAGS.init_lr = 0.2 * FLAGS.init_lr
                print('Learning rate decayed to ', FLAGS.init_lr)

            if step == FLAGS.decay_step2 or step == FLAGS.decay_step3:
                #FLAGS.init_lr = ((step / FLAGS.train_steps)**0.9) * FLAGS.init_lr
                FLAGS.init_lr = 0.5 * FLAGS.init_lr
                print('Learning rate decayed to ', FLAGS.init_lr)

            if step == FLAGS.decay_step4:
                #FLAGS.init_lr = ((step / FLAGS.train_steps)**0.9) * FLAGS.init_lr
                FLAGS.init_lr = 0.7 * FLAGS.init_lr
                print('Learning rate decayed to ', FLAGS.init_lr) 

            # Save checkpoints every 10000 steps
            if step % 20000 == 0 or (step + 1) == FLAGS.train_steps:
    ...
    def test(self, test_image_array):
        ...
        logits = designdeNetwork(self.test_image_placeholder, reuse=False)
        predictions = tf.nn.softmax(logits)
        ...
        if remain_images != 0:
            ...
            # Build the test graph
            logits = designdeNetwork(self.test_image_placeholder, reuse=True)
            predictions = tf.nn.softmax(logits)
